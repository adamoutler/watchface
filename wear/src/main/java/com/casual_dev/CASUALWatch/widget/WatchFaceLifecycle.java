 ...
 * <p/>
 * Modified by Adam Outler 1Nov14- removed duplicate events
 */
public class WatchFaceLifecycle {

    final static String TAG = WatchFaceLifecycle.class.getSimpleName();
        ...
        public void onDisplayChanged(int displayId) {
            ...
            switch (state) {
                case Display.STATE_DOZING:
                    ...
                    Log.d(TAG, "Watch: State Dozing" + display.getState());
                    break;
                case Display.STATE_OFF:
                    Log.d(TAG, "Watch: State Off");

                    listener.onScreenOff();
                    break;
                case Display.STATE_ON:
                    Log.d(TAG, "Watch: State On");
                    listener.onScreenAwake();
                    break;
                case Display.STATE_UNKNOWN:
                    Log.d(TAG, "Watch: State Unknown");
                    //  Not really sure what to so about Display.STATE_UNKNOWN, so we'll do nothing at all
                    break;
                default:
                    Log.d(TAG, "Unhandled Display State:" + display.toString());
                    //catch-all for future additions v
                    break;
            }
        }
